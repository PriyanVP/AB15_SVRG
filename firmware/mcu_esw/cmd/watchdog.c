...
// Variable to store WD periodicity [n*100us]
...
void CmdConfigureWatchdog(USBReceiveData const * const commandPackage)
{
    ...
    g_wdPeriodicity20UsTicks = (uint16)data[3];
...
uint16 CalculateWatchdogAckPeriodicity(void)
{
    ...
    // maybe     IfxGpt12_T2_getMode(gpt12) or similar can be used

    //IRQ freq = (GENERAL_TIMER_PERIODICITY * 16 * 1)
    // duration = 1/f
    // durationUs = 1/f *1000000   or 1000000/f

    durationOfTimerInterruptUs = (GENERAL_TIMER_PERIODICITY * 16 * 1) / (100); // Configured duration of General Timer interrupt, microseconds; 1/100 = (s->us constant)/(FREQ_GPT12_HZ)

    // Responsetime in factor of 100uS ( value 5 = 500us) max val of 65535 will result of timer of ~6,5s
