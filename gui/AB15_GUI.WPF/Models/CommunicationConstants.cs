...
//    RESET_TO_IDLE               = 20,                 /** \brief stop all continuous operations, set all pins to default state, wait for new command */
    GET_CONFIG_WATCHDOG         = 21,                 /** \brief get values of Watchdog config registers from CS600, and send them to PC to populate corresponding GUI fields */
    START_MONITORING_WATCHDOG   = 22,                 /** \brief enables MCU functionality that sends values of Watchdog status registers and time durations between the polls to GUI; continuous command */
    STOP_MONITORING_WATCHDOG    = 23,                 /** \brief disables MCU functionality that sends values of Watchdog status registers and time durations between the polls to GUI */
//    BYPASS_MCU                  = 24,                 /** \brief sets MCU to bypass mode (not interfering external MCU communication) */
//    ACTIVATE_MCU                = 25,                 /** \brief activates MCU, exits from bypass mode */
    GET_MCU_VERSION             = 26,                 /** \brief returns MCU SW version */
    GET_MCU_BUILD_DATE          = 27,                 /** \brief returns MCU SW build date */
    GET_MCU_BUILD_TIME          = 28,                 /** \brief returns MCU SW build time */
